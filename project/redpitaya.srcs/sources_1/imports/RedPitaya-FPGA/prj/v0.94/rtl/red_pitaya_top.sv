...
wire [DWE-1:0] exp_p_dtr_gated;
assign exp_p_dtr_gated[0]         = 1'b0;  // force input
assign exp_p_dtr_gated[1]         = 1'b0;  // force input
assign exp_p_dtr_gated[DWE-1:2]   = exp_p_dtr[DWE-1:2];  // rest unchanged

IOBUF i_iobufp [DWE-1:0] (.O(exp_p_in), .IO(exp_p_io), .I(exp_p_otr), .T(~exp_p_dtr_gated) );
...
// -------------------------------------------------------------------------
// CLEANED STRUCTURAL STUB BLOCKS
// -------------------------------------------------------------------------
// Generates stubs ONLY for completely unassigned system bus lines.
// sys[6] is skipped globally here so your TDC owns it exclusively!
generate
for (genvar i=7; i<8; i++) begin: for_sys_unassigned
  sys_bus_stub sys_bus_stub_inst (sys[i]);
end: for_sys_unassigned
endgenerate

`else // ====================================================================
      // ? SCOPE_ONLY BRANCH ACTIVATION
      // ====================================================================
...
IBUFDS #(.IOSTANDARD ("DIFF_HSTL_I_18")) i_IBUFGDS_clk (.I(daisy_p_i[1]), .IB(daisy_n_i[1]), .O(rxs_clk));
IBUFDS #(.DIFF_TERM ("FALSE"), .IOSTANDARD ("DIFF_HSTL_I_18")) i_IBUFDS_dat (.I(daisy_p_i[0]), .IB(daisy_n_i[0]), .O(rxs_dat));
OBUFDS #(.IOSTANDARD ("DIFF_HSTL_I_18"), .SLEW ("FAST")) i_OBUF_clk (.O(daisy_p_o[1]), .OB(daisy_n_o[1]), .I(1'b0));
OBUFDS #(.IOSTANDARD ("DIFF_HSTL_I_18"), .SLEW ("FAST")) i_OBUF_dat (.O(daisy_p_o[0]), .OB(daisy_n_o[0]), .I(1'b0));
...
// =========================================================================
// ? DUAL-CHANNEL QUANTUM TIME-TAGGER CORRIDOR INTERCONNECT
// =========================================================================
// Bound strictly outside macro loops to protect unique bus architecture
hydraharp_replica_top #(
    ...
    .spad_ttl_ch0  (exp_p_in[0]     ), // Channel 0: E1 Extension Pin 3 (DIO0_P)
    .spad_ttl_ch1  (exp_p_in[1]     ), // Channel 1: E1 Extension Pin 5 (DIO1_P)
